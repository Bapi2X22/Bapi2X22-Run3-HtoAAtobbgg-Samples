...
base_dir = "InputCards"
base_name = "Wh_M125_ToAA_15_Tobbgg"
...
for mA in mass_points:
    ...
    for fname in os.listdir(base_dir):
        ...
        new_fname = fname.replace("15", f"{mA}")
...
""")
        ...
        # Modify proc_card.dat
            ...
            with open(src, "r") as f:
                content = f.read()
            # Replace output line
            new_output_line = f"output Wh_M125_ToAA_{mA}_Tobbgg"
            lines = content.splitlines()
            for i, line in enumerate(lines):
                if line.strip().startswith("output Wh_M125_ToAA_"):
